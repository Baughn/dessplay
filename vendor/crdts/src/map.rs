...
impl<K: Ord, V: Val<A>, A: Ord + Hash + Clone> Map<K, V, A> {
    ...
    // DESSPLAY PATCH: read-only scans must not clone unused causal clocks.
    /// Iterate over borrowed keys and values without allocating causal contexts.
    /// Use [`Self::iter`] when deriving an add or remove context from each entry.
    pub fn iter_entries(&self) -> impl Iterator<Item = (&K, &V)> {
        self.entries.iter().map(|(key, entry)| (key, &entry.val))
    }
