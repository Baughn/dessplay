...
pub struct UdpSocketState {
    ...
    /// DESSPLAY PATCH: the socket-level TOS / traffic-class byte at
    /// creation, ECN bits masked off. The per-packet `IP_TOS` /
    /// `IPV6_TCLASS` cmsg overrides the socket option, so upstream's
    /// ECN-only cmsg value zeroes any DSCP set via `setsockopt`; ORing
    /// this base back in preserves it. (Upstream declined DSCP support:
    /// quinn-rs/quinn#1749.)
    tos_base: u8,
}
...
impl UdpSocketState {
    pub fn new(sock: UdpSockRef<'_>) -> io::Result<Self> {
        ...
        // DESSPLAY PATCH: capture the socket's TOS / traffic-class byte so
        // per-packet ECN cmsgs preserve its DSCP bits (see `tos_base`).
        // Best-effort: a failed getsockopt means an untagged socket.
        let tos_base = {
            ...
            // `val` starts zeroed, so a 1-byte result (BSD-style IP_TOS)
            // reads correctly through the c_int on little-endian targets.
            if rc == 0 {
                (val as u8) & !0b11
            } else {
                0
...
fn prepare_msg(
    ...
    encode_src_ip: bool,
    sendmsg_einval: bool,
    // DESSPLAY PATCH: socket-level DSCP bits to preserve in the TOS cmsg.
    tos_base: u8,
) {
    ...
    // DESSPLAY PATCH: the cmsg overrides the socket-level TOS byte, so it
    // must carry the socket's DSCP bits, not the ECN codepoint alone.
    let tos = tos_base as libc::c_int | transmit.ecn.map_or(0, |x| x as libc::c_int);
