...
4. [Gossip Messages](#gossip-messages)
...
When any node (client or server) receives a remote CRDT op via `on_remote_op`,
it relays the op to all other connected peers via `BroadcastControl`. The
sender is excluded from the broadcast (handled by `dispatch_sync_actions`).
CRDT idempotency (`apply_op` returns false for duplicates) prevents relay
loops. This ensures state changes propagate even without direct P2P
connectivity — the rendezvous server acts as a relay for CRDT ops.
...
Playback position and seek are **ephemeral gossip messages** — a separate
category from CRDT ops (persistent, reliable) and QUIC datagrams (P2P-only).

### Design

Gossip messages:
- Carry an **origin UserId** and **timestamp** (for dedup/LWW)
- Are sent to **all connections** (P2P peers + rendezvous server)
- Are **forwarded** by any receiver to its other connections
- Are **never persisted** (not in SQLite, not in the op log)
- Use **LWW dedup**: drop if we already have a newer-or-equal timestamp
  from this (origin, message-type) pair

### Message Types

| Type | Interval | Description |
|------|----------|-------------|
| `Position` | 100ms playing, 1s paused | Current playback position |
| `Seek` | Debounced 1500ms | Seek target after user scrubs |

### Forwarding

When a node receives a gossip message (from any source — P2P datagram,
server relay, or peer control):

1. Check `GossipState::accept(&msg)` — if stale/duplicate, drop
2. Process locally (update `remote_positions`, emit seek, etc.)
3. Forward to all other connections, excluding the source

The rendezvous server participates as a relay: it accepts gossip from clients
and forwards to all other connected clients. Combined with peer-to-peer
forwarding, this ensures messages reach all nodes in a connected graph even
without full mesh P2P connectivity.

### Why Not CRDT Ops?

Position updates at 10 ops/sec would overwhelm the op log and SQLite storage.
Gossip messages are fire-and-forget: if one is lost, the next one (100ms later)
replaces it.
