...
**Status (2026-06-28): deferred (documented), by user decision.** This is the same finding as the download-region duplicate below ("Downloading unpause condition enforces only the 20% half"). The throughput half is intentionally **not** implemented: `design.md` (File State) now records that only the 20% half is enforced today and lists the download-speed-vs-bitrate half under Future Plans (it needs a synced eligibility signal on `FileAvailability::Downloading`). No code change — the impact is a self-only edge that never gates the group.
...
**Status (2026-06-28): fixed (test added).** `manual_pause_overrides_not_watching` (`dessplay-core/src/derive.rs`) now also asserts the gating side: `playback_blockers(&view, &[present("kim")])` is empty and `playback_active(&view, &[present("kim")])` is true for the present NotWatching+Paused user — covering the precedence inversion (the manual Paused override wins for `user_state` *display*, but `playback_blockers` short-circuits NotWatching to None before the manual-pause check, so it never gates). Production logic was already correct (this was purely a coverage gap), so no production code changed. **Mutation sanity-check:** temporarily replacing the `SeriesWatchState::NotWatching => None` arm in `playback_blockers` with the Maybe-style present/absent match (i.e. removing the short-circuit so a present NotWatching+Paused user blocks) made the new `playback_blockers(...).is_empty()` assertion FAIL; reverting restored it to green.
...
**Status (2026-06-28): fixed.** `committed_absent_blocker` (`dessplay/src/ui/props.rs`) now returns false when the user's `manual_override` resolves to `ManualState::Away { .. }`, mirroring the early `continue` in `derive::playback_blockers` exactly — so an Away-excused committed-absent user falls through to the dim departed line (or, for Lost, the greyed "lost" row) instead of a red "committed, away" blocker, and the Users pane agrees with gating again. Only the Away override excuses (a Paused override on an *absent* user still yields `CommittedAbsent` in `playback_blockers`, so no Paused parity is needed). Regression test `away_excuses_a_committed_absent_user_in_the_users_pane` (props.rs) covers both Lost and Departed: it asserts the user starts as a `Tone::Blocked` blocker, then — after the Away override — that no red blocker row remains. The test FAILED before the fix (the away-excused user still rendered "committed, away" / `Tone::Blocked`) and passes after.
...
**Status (2026-06-28): fixed (doc).** Corrected the `playlist.rs` module doc to name the real compaction path: the server keeps the rationals bounded at its daily compaction by rebuilding the playlist from resolved order with flat `0, 1, 2, ...` identifiers via [`CrdtState::push_playlist_entry`] (`crate::compact::rebuild`). **Kept** `rebalance_playlist` (option (a)) rather than deleting it: it does the same reassignment *in place* and is the only thing the playlist property test, the `playlist_identifier` fuzz target, and a unit test exercise for the `Identifier::between`-based rebalance logic — that coverage is not redundant with `compact::rebuild` (which appends fresh entries rather than rebalancing existing positions), and the method is a plausible future mechanism. The doc now notes it is test-exercised but not the live compaction path. Comment-only; no code change.
...
**Status (2026-06-28): fixed (test added).** Extended `cache_recomputes_only_when_inputs_change` (`dessplay-core/src/franchise.rs`): after the cache is built and after the existing metadata-change recompute (`recomputes == 2`), it now applies a **series_relations-only** change — a `Sequel` edge on Overlord (10816) pointing at the already-file-bearing KonoSuba (11261), with no metadata touched — and asserts the cache recomputes (`recomputes == 3`) *and* that the grouping reflects the merge: one franchise instead of two, holding both file-bearing seasons (`series == [10816, 11261]`) and both files. This covers the relations half of `inputs_fingerprint`, modelling the realistic case where the relations graph fills in over hours while file metadata is already settled. Production code unchanged (coverage gap only). **Mutation check:** temporarily dropping the `series_relations` loop from `inputs_fingerprint` made the new assertions FAIL (the cache reused the stale two-franchise grouping — `merged.len()` was 2, never recomputing) while the pre-existing metadata/position assertions still passed; reverting restored green.
...
**Status (2026-06-28): fixed (doc).** Changed the FileId Key-Definitions wording in `docs/design.md` from "an exact multiple of the 9,728,000-byte block size" to "an exact **non-zero** multiple", matching the (correct) code and the precise `hash.rs` module doc. A 0-byte file hashes to MD4 of the empty string with no trailing empty-block hash; the prior literal reading (zero is a multiple) would have prescribed a non-AniDB-compatible hash for a reimplementer. Doc-only; the code was already correct, so no code change.
...
**Status (2026-06-28): fixed (doc).** Rewrote the `WireMessage` doc comment (`dessplay-core/src/net/message.rs`) to state that relay envelopes are **not** a `WireMessage` variant -- they are framed as `RelayEnvelope` (in `net::transfer`) on a dedicated relay stream, kept off the control channel so bulk transfer never head-of-line-blocks state sync -- pointing at docs/network-design.md (Transfer Stream / Relay Envelope). Comment-only; no code or enum change.
...
**Status (2026-06-28): fixed.** Moved `set_priority(CONTROL_PRIORITY)` into `QuicTransport::new` (`dessplay-core/src/net/quic.rs`), so *both* endpoints elevate their control send stream on construction — the server's accepted control stream now sits above its bulk relay/transfer streams, matching the client. The now-redundant client-side call in `QuicConnector::connect` was removed (the single construction site covers both). It *was* unit-testable after all: the new in-module real-localhost-QUIC test `both_endpoints_prioritize_the_control_stream` connects a `QuicConnector` to a `QuicListener` and reads the priority back off each side's control send stream via quinn 0.11's `SendStream::priority()` getter, asserting both equal `CONTROL_PRIORITY` (the in-module `#[cfg(test)]` submodule reaches the private `control_send` field directly, so no public/test-support surface was added). **Mutation check:** restoring the historical client-only wiring (drop the `new` call, re-add the connect-path call) made the test FAIL on the *server* assertion while the client assertion still passed — confirming it pins the exact server-side gap; reverting restored green. Existing `quic_localhost` integration tests still pass.
...
**Status (2026-06-28): deferred (documented), by user decision.** Duplicate of the gating-region finding ("Downloading unpause rule enforces only the 20% half"). Resolved by documenting rather than implementing: `design.md` now states only the 20% half is enforced and lists the throughput half under Future Plans. No code change.
...
**Status (2026-06-28): deferred (documented).** Partial-serve is part of the **same full multi-source/partial-serve transfer scope that was deliberately left unimplemented** in the minimal single-source fix — see the HIGH "Only one source is ever solicited" finding above, whose Status notes that "true multi-source *concurrency*, rarest-first load-balancing across sources, and partial-serve from still-downloading peers remain **unimplemented**". The user has chosen to keep the download work minimal for now, so this is intentionally not implemented and the `ChunkStore::available`/`read_chunk` scaffolding is kept for that planned future work. Both are `pub fn` (not warned as dead code; `cargo clippy -p dessplay --lib` is clean), so no code change was made. If/when partial-serve lands it would also satisfy network-design.md's "Updated when a downloading peer completes new chunks" / "those leechers can then serve each other"; until then the one-seeder/relay-through-NAS scale makes complete-copy-only serving acceptable.
...
**Status (2026-06-28): fixed.** Note the cited line numbers and the `Pending { peer, requested_at }` state predate the single-source MEDIUM fix (2026-06-27), which replaced the lone-peer tracking with a per-source `Source::solicited` flag and made `progress_and_refill` solicit *every* source at once. In that post-fix code the residual bug was that an invalid-block-hash source was **left in the source set**: it kept `solicited` latched true (so it was never usefully re-asked) yet still held its advertised bitfield, so once a *good* source validated the hashes the scheduler requested **chunks from the liar's bogus bitfield** — looping on the bad peer and wasting the relay uplink (a single bad source also wedged silently with no re-solicitation). The fix: in `on_block_hashes`, on a `block_hashes_match` failure, **drop the bad source** (`d.sources.remove(&from)`) — mirroring the snub source-drop, so its bogus bitfield is discarded and it is never picked for chunks — then `progress_and_refill` to re-solicit a *different* source. If it is still a synced Ready candidate, `set_sources` re-adds it fresh next refresh (exactly as for a snub): an honest peer that transiently erred gets another chance, while a persistently-broken one self-limits to one ask per refresh and never blocks a good source. Abandon-on-exhaustion was deliberately **not** added (it risks a recreate→abandon loop against the per-snapshot `StartDownload` re-emit, and a no-source download already waits harmlessly until a good source appears — the minimal, spec-consistent choice). Regression test `an_invalid_block_hash_source_is_dropped_and_not_asked_for_chunks` (download.rs) reproduced the bug (the liar was sent `ChunkRequest [0..15]`) and passes after; the end-to-end guard `download_completes_after_a_source_serves_invalid_block_hashes` covers the re-solicit-and-complete path.
...
**Status (2026-06-28): fixed.** The divergence path (`dessplay/src/actors/sync.rs`) now emits `SyncEvent::Diverged` via `try_send` (dropped when full) instead of a blocking `send().await`, matching `changed()`'s non-blocking contract — a stalled/absent consumer can no longer wedge the sync actor on the Diverged signal. The needed `RequestMerge` is still sent first (unconditionally, on the net channel), so dropping the Diverged UI hint when the events channel is full loses nothing essential; the divergence recovery proceeds either way. Regression test `divergence_never_wedges_a_stalled_consumer` (sync.rs) mirrors `slow_ui_consumer_never_wedges_the_sync_actor`: it leaves the events channel full and undrained (200 flooded remote ops), fires two consecutive hash mismatches, and asserts the actor still answers `GetView` within a (paused-time) 30s budget. Pre-fix the actor wedged and the query timed out (`Elapsed`); post-fix it answers immediately. The existing `divergence_alarm_after_two_mismatches` (which drains events) still observes the Diverged signal, confirming the signal is preserved when the consumer keeps up.
...
**Status (2026-06-28): fixed (client-side no-op).** `play_selected` now early-returns an empty action list when `self.snapshot.view.now_playing == Some(hash)`, so re-selecting the already-playing entry emits no `SetNowPlaying` at all and the server never resets seek authority for a non-transition. (Chose the client-side guard over the server-side value-change gate — it is the narrower change and keeps the EOF/manual-select transition semantics untouched.) The existing test `selecting_now_playing_entry_does_not_pause` was tightened and renamed to `reselecting_now_playing_entry_is_a_noop`: it now asserts the re-select yields `vec![]` (was `[SetNowPlaying]`, which is how it FAILED pre-fix), while the different-file select still emits `SetNowPlaying` + intent Paused.
...
**Status (2026-06-28): fixed (code corrected to match the doc).** The `Msg::ToggleAway` handler's `currently_away` check now binds `set_by` and only treats the Away as clearable when `*set_by == self.me`; an Away set by anyone else falls through to the re-mark branch (Away `set_by` me) instead of being cleared. So `a` only clears an Away you set yourself, matching design.md's keyboard table ("clear an Away you set"); another user's Away is cleared only by the marked user's own "I'm here" action. New regression test `users_pane_a_does_not_clear_anothers_away` (ui_app.rs): baughn Away set by nero, me=kim presses `a` → emits `SetManualOverride{Away{set_by: kim}}`, not `None` (confirmed FAILING with the old `{ .. }` match). The existing `users_pane_toggles_away` (Away set by me clears; not-away marks) still passes.
...
**Status (2026-06-28): fixed.** Aligned the List edit modal's save UX with the SettingsModal pattern: `on()` now saves on capital `S` (`typed(ev) == Some('S')`) and on Enter on a new selectable `[Save]` row (appended after the fields; `save_index()` = `LIST_FIELDS.len()`, and the Down cap was raised from `len-1` to `save_index()` so the row is reachable). Ctrl-s is kept as a working alias but no longer advertised; `keybindings()` now shows `("Enter", "Edit/Cycle"), ("S", "Save"), ("Esc", "Cancel")`. The save-message construction was extracted into `save_msg()` (shared by all three paths). Regression tests: `list_edit_modal_saves_via_capital_s_and_save_row` (modals.rs — capital `S`, the `[Save]` row via Enter, and Ctrl-s alias all yield `ListEntrySaved`; confirmed FAILING for capital-`S`/save-row before the fix) and the end-to-end `list_edit_modal_saves_on_capital_s` (ui_app.rs — capital `S` routes through the dispatcher, emits `PutListEntry`, and closes the modal). The existing Ctrl-s tests still pass.
...
**Status (2026-06-28): fixed (doc corrected; code was right).** The code's bare-letter `J`/`K` (and `j`/`k`) reorder is the intended, terminal-safe choice (Ctrl-J == LF collides in terminals lacking the enhanced keyboard protocol), consistent with the playlist pane's documented reorder keys — so design.md was the stale side. Rewrote design.md's Settings-screen bullet to `J`/`K` (or `j`/`k`) with the Ctrl-collision rationale. Added a lock-in regression test `media_roots_reorder_with_shift_jk` (modals.rs) asserting `J`/`k` swap roots and carry the cursor, so doc and code can't silently drift apart again.
...
**Status (2026-06-28): fixed.** The doc/help was the stale side — the production default is 48 (`run::download_config` does `unwrap_or(48)`, the `HeadlessArgs` field doc says 48, and `pipeline_depth_flag_overrides_default` already pins it at 48). Corrected three sites to say 48: the clap `--pipeline-depth` help (`main.rs`), the `download_config` doc comment (`run.rs`), and the `DownloadConfig::pipeline_depth` field doc (`download.rs`), which had misattributed the struct's `Default` of 16 (test-only) to the flag. `DownloadConfig::default()` keeps 16 (used only by tests via `..Default::default()`). Regression test added (`main.rs` `pipeline_depth_help_states_the_production_default`): renders the clap help (line-wrapping normalized) and asserts it states `default 48`, not `default 16`. Confirmed failing before the fix (help read "default 16") and passing after.
...
**Status (2026-06-28): fixed.** `SeederTransfer::on_state` (`dessplay/src/seeder.rs`) now stages `StartDownload` in unwatched-first order via a new pure helper `download_order(view)`: it stable-sorts `view.playlist` by the group watched flag (`false` < `true`), so unwatched entries are fetched ahead of watched back-catalog while preserving playlist order within each group (the resolve loop still walks plain playlist order — order doesn't matter there). Regression test `download_order_puts_unwatched_first_then_playlist_order`: a playlist `[1,2,3,4]` with 1 and 3 watched yields download order `[2,4,1,3]`; confirmed failing under a mutation that drops the sort (it produced `[1,2,3,4]`) and passing with it.
...
**Status (2026-06-28): fixed.** `placeholder_lines` (`dessplay/src/session.rs`) now classifies each interactive peer's derived state into two buckets — `Watching` = {`Ready`, `Maybe`} (actively watching), and a new `Not watching:` line = {`Paused`, `Away`, `NotWatching`} — instead of labeling everyone-but-NotWatching as "Watching". The `Not watching:` line is omitted when empty; seeders stay excluded. Regression test `placeholder_lists_who_is_and_isnt_watching`: with ann (Maybe), ben (Paused), cat (Away), dan (NotWatching) and a seeder, the placeholder reads `Watching: ann` then `Not watching: ben, cat, dan`. Confirmed failing before the fix (it produced `Watching: ann, ben, cat` and no not-watching line) and passing after.
...
**Status (2026-06-28): fixed.** Paths are now stored as the OS-native bytes in BLOB columns instead of `to_string_lossy()` into TEXT, so non-UTF-8 paths round-trip losslessly. Added `path_to_bytes`/`path_from_bytes` helpers (`dessplay/src/storage.rs`): on Unix the raw `OsStr::as_bytes()`, with a UTF-8 fallback on non-Unix (this project targets Linux). All six write sites and their read sites were converted (media_roots.path, cache_entries.path, hash_cache.path incl. the `remove_hash_cache` DELETE key, manual_mappings.local_path, series_map_dirs.dir). Schema migrated with a new append-only **v3** migration (bumps `PRAGMA user_version` to 3) that rebuilds those five tables with the path column as `BLOB` and copies existing rows via `CAST(path AS BLOB)` — existing rows are UTF-8, whose CAST bytes equal `OsStr::as_bytes()`, so they reconstruct identically (SQLite can't change a column type in place, and STRICT rejects a BLOB in a TEXT column, hence the rebuild). The migration shape was **not** ambiguous (existing rows are all valid UTF-8; the copy is mechanical), so no BLOCK was needed. Two regression tests: `non_utf8_paths_round_trip_losslessly` round-trips a `\xff\xfe` path through every path column (and proves two distinct non-UTF-8 paths that collapse to the same lossy string both persist without violating `media_roots UNIQUE`) — confirmed failing under a lossy-`path_to_bytes` mutation, passing after; and `v3_migration_preserves_existing_utf8_paths` verifies a v2 DB's UTF-8 rows survive the upgrade. (The verification trail's note that `remove_hash_cache` "won't match the stored key" was a self-mismatch overstatement — the real risk is cross-path collision — but the BLOB key fix covers both.)
...
**Status (2026-06-28): fixed.** Took the first option: dropped the `Instant::now()` reads and the `elapsed_ms` debug-log field from both `save_state` and `load_state` (`dessplay/src/storage.rs`), so the storage layer truly never reads the clock — matching the module doc and design.md, Schema. The caller-supplied `now: i64` that `save_state` persists is unchanged; the remaining `epoch`/`bytes` debug fields stay. **No regression test added (untestable behaviorally):** the removed clock read fed only a debug-log field that was never stored nor asserted, so it had no observable effect on stored data or test output (`tracing-test` is not a dev-dependency of this crate, and adding one just to assert a log field's absence isn't warranted). The fix is verified by inspection — the only remaining `Instant::now()` mention in the file is the explanatory comment.
...
**Status (2026-06-28): fixed.** The entry-build `notes` collection (`dessplay/src/import.rs`) now filters out the column the section relabel pointed `status_note` at (`.filter(|&&index| Some(index) != columns.status_note)`), so a "Progress?" cell that is also a notes column (the Ivory sheet's "Extra Notes", idx 6) is read **once** as the status note and never duplicated into `notes`. Filtering at build time (rather than mutating `columns.notes`) auto-restores the column's notes role on a later section that clears `status_note` — satisfying the "restore it to notes" requirement for free. Regression test: `planning_sheet_sections` now also asserts `shirobako.entry.notes.is_empty()` (Shirobako's idx-6 cell "13" landed in both `notes` and `status_note` before; the assertion FAILED pre-fix, passes after).
