...
reliability over flaky connections. Server-coordinated with optional
...
4c. By default: The file is retrieved from peers using a bittorrent-like protocol.
    It is kept in a temporary directory until at least 50% of the file has been
    watched, at which point it is moved into [Series name]/[Season #]/[Original
    filename] in the download root, aka. the topmost media root.
...
2. **Manual override** (`LWWReg<Option<ManualState>>`): The user can manually
   ...
   is cleared when the user explicitly resumes.
...
1. **Play** only proceeds when every user is Ready or Not Watching, and their File
   State permits playback
...
- **Topology**: Hub-and-spoke for state sync (all traffic through server);
  optional peer-to-peer for file transfer only
...
2. **Peer list distribution**: Clients receive list of other peers (for file transfer)
3. **STUN**: Tell clients their observed IP:port (for peer-to-peer file transfer)
4. **Relay**: Forward file transfer traffic between clients who cannot connect directly
5. **Compaction**: Replaying the CRDT op logs into snapshots and incrementing the epoch
6. **AniDB lookups**: Enriching playlist items with series/season/episode metadata
7. **Authoritative actions**: EOF -> next file transitions, server actor ID for seek authority
...
|------|-----------|-------|
| Playlist | `Map<Ed2kHash, MVReg<Lww<PlaylistFileState>>>` | `Identifier`-based ordering |
...
| Manual override | `Map<UserId, MVReg<Lww<Option<ManualState>>>>` | Per user |
...
| Chat | `GList<ChatMessage>` | Grow-only ordered list |
| Playback position | `Map<UserId, MVReg<Lww<PlaybackPosition>>>` | Per user, high frequency |
...
the video plays iff every user's state is Ready or Not Watching, and
...
- A subtitle pane, showing recent subtitles from the video. Optionally fused
  with the chat pane.
